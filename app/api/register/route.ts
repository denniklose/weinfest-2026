...
export async function POST(request: Request) {
  ...
  catch (error) { if (error instanceof Error && error.message === "NAME_TAKEN") return NextResponse.json({ error: "Dieser Name wurde bereits verwendet. Wende dich bitte an Elana oder Tom." }, { status: 409 }); if (error instanceof Error && error.message === "NUMBER_TAKEN") return NextResponse.json({ error: "Diese Wein-Nummer wurde bereits verwendet. Prüfe dein Namensschild oder wende dich an Elana oder Tom." }, { status: 409 }); return NextResponse.json({ error: "Der Zugang konnte nicht geöffnet werden." }, { status: 500 }); }
}
